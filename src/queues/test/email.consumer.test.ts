...
jest.mock('amqplib');
jest.mock('@sumaniac28/gigglobal-helper');
...
describe('Email Consumer', () => {
  ...
  describe('consumeAuthEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('GigGlobal-email-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('auth-email-queue', 'GigGlobal-email-notification', 'auth-email');
    });
  });
  ...
  describe('consumeOrderEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('GigGlobal-order-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('order-email-queue', 'GigGlobal-order-notification', 'order-email');
    });
  });
});
